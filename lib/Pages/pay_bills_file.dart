...
class PayBillsFileState extends State<PayBillsFile> {
  ...
  var selectedBill;
  var billItems;
  int curShowPage = 0; //0: categories, 1: billers, 2: options, 3: payment, 4: complete
  ...
  void refresh() async {
    ...
    try {
      ...
      Map tmpCats = await _interswitch.getCategories();
      if (tmpCats.containsKey("error")) {
        print(tmpCats['error']);
        CommonUtils.errorToast(context, tmpCats['error']['message'] ?? "Something went wrong");
      } else {
        List cats = tmpCats['categorys'];
        if (cats != null) {
          cats.sort((a, b) => a['categoryname'].toString().compareTo(b['categoryname']));
        }
        setState(() {
          categories = cats;
        });
  ...
  getData() {
    if (curShowPage == 0)
      return categories;
    else if (curShowPage == 1)
      return billers.where((element) => element['categoryid'] == selectedCat).toList();
    else
      return [];
  }
    ...
    return Container(
      ...
      child: InkWell(
        ...
        onTap: () {
          if (curShowPage == 0) {
            setState(() {
              selectedCat = billItem['categoryid'];
              curShowPage = 1;
            });
          } else {
          ...
          child: Row(
            children: [
              ...
              Expanded(
                ...
                child: Text(
                  billItem[curShowPage == 1 ? 'billername' : 'categoryname'] ?? "",
  ...
  _renderBillsList() {
    ...
    return Container(
      ...
      child: ListView.builder(
        ...
        itemBuilder: (BuildContext context, int index) => renderItem(listData[index]),
      ),
    );
  }

  Widget _renderField(Map item, String key) {
    if (item == null || !item.containsKey(key) || item[key] == null) return Container();
  ...
  strMenuItem(var item) {
    try {
      ...
      return name + " (" + currency + tmpamount.toString() + ")";
    } catch (e) {
      return "";
    }
  }
  ...
  _renderOptions() {
    ...
    if (selectedBillOption == null) {
      ...
      amountController.text = (double.parse(selectedBillOption['amount']) / 100).toString();
    }
    return Container(
            ...
            child: DropdownButton<String>(
              ...
              onChanged: (String newValue) {
                setState(() {
                  ...
                  amountController.text = (double.parse(selectedBillOption['amount'].toString()) / 100).toString();
                });
              },
              ...
              items: tmpBillItems.map<DropdownMenuItem<String>>((var value) {
  ...
  _body(BuildContext context) {
    return Container(
      ...
      child: SingleChildScrollView(
        child: (curShowPage == 0 || curShowPage == 1)
            ? _renderBillsList()
            : curShowPage == 2 || curShowPage == 3
                ? _renderBillOptions()
                : _renderBillComplete(),
      ),
    );
